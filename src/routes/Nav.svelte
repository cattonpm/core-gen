    ...
    onMount(() => {
		...
		connectToMetamask = async function() {
            ...
            try {
                ...
                if (error.code === 4902) {
                    // Core has not been added, we will add here
                    try {
...
<!--Nav bar-->
    <div class="flex justify-between items-center lg:gap-[30px] gap-[10px] py-2 z-30 text-sm ">
        <!--Logo-->
        <div class="flex items-center lg:gap-[30px] gap-[10px]">
            ...
            <div class="lg:flex hidden items-center  md:gap-[20px] gap-[3px]"> 
                ...
                <button class=" text-white font-semibold hover:text-buttonHover" on:click={() => window.open('https://corearena.xyz/')}>
                    PvP Game
                </button>
            </div>
        </div>
        ...
        <!--items-->
        <div class="flex items-center sm:gap-2 gap-[20px] pr-[10px]">
                    <!--Big screen items-->
            <div class="lg:flex hidden items-center gap-[10px] mr-[4px] font-semibold">      
                <div class="relative flex flex-col">
                    <div class=" font-md text-white px-4 rounded-md pb-[1px]">
                        <img src="/coredao.svg" class="h-[25px]" alt="Core chain"/>
                    </div>
                </div>
            </div>
        ...
        <div class="flex flex-col sm:w-[300px] w-full min-h-screen px-4 pt-4 self-end bg-greenYellow overflow-y-scroll hide-scrollbar " in:slide={{ duration: 300, y: 300 }} out:slide={{ duration: 300, y: 300 }}>
            <div class="flex items-center justify-between w-full bg-white shadow-md rounded-lg">
                <div class="flex flex-grow rounded-lg items-center bg-white">
                    ...
                    <input bind:value={searchText} class=" w-full bg-white" placeholder="  Seach by collections/contracts"
                    on:input={() => {
